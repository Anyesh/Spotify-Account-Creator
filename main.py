...
class Main:
    ...
    def error_import(self, create):
        ...
        print(
            "%s> %sPaste your proxies inside Proxies.txt%s!" % (reset(), red(), reset())
        )
        system("title [Spotify Account Creator] - Exiting . . .")
        sleep(3)
        _exit(0)
    ...
    def write(self, arg):
        ...
        )  # Get less printing bugs on Windows
        lock.release()
    ...
    def update_title(self):
        if self.variables["unlimited"]:
            while True:
                ...
                system(
                    "title [Spotify Account Creator] - Created: %s ^| Retries: %s ^| CPM: %s ^| Time Elapsed: %s ^| Threads: %s"
                    % (
                        self.variables["created"],
                        self.variables["retries"],
                        self.variables["cpm"],
                        elapsed,
                        (active_count() - 2),
                    )
            ...
            while self.variables["created"] != int(self.amount):
                elapsed = strftime("%H:%M:%S", gmtime(time() - self.start))
                system(
                    "title [Spotify Account Creator] - Created: %s/%s ^| Retries: %s ^| CPM: %s ^| Time Elapsed: %s ^| Threads: %s"
                    % (
                        self.variables["created"],
                        self.amount,
                        self.variables["retries"],
    ...
    def get_email_from_list(self):
        emails = []
        if path.exists("email_list.txt"):
        ...
        if self.variables["unlimited"]:
            while True:
                try:
                    Thread(
                        target=self.creator,
                        args=(self.variables["proxies"][self.variables["proxy_num"]],),
                    ).start()
                except:
                    continue
